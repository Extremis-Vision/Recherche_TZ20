...
def check_lmstudio_connection(model: str = "gemma-3-12b-it-qat"):
    client = OpenAI(
        base_url="http://localhost:1234/v1",
        api_key="not-needed"
    )
...
def RAG(contenu : str , model : str =  "gemma-3-12b-it-qat"):
    # Vérifier la connexion à LM Studio
    if not check_lmstudio_connection(model):
    ...
    try:
        ...
        response = rag_with_lmstudio(question, collection, model)
